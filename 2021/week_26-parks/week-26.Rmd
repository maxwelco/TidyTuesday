...
theme_style <- theme(#text = element_text(family = font_family),
                  ...
                  plot.caption = element_text(size = 7, colour = text_colour),
...
parks2 %>% 
  ...
  geom_text(data = parks2 %>% filter(year == "2015"),
            mapping = aes(label = city, x = 2014.9, color = variable_col),
            hjust = "right",
            fontface = "bold",
            size = 3) +
  geom_text(data = parks2 %>% filter(year == "2020"),
            mapping = aes(label = city, x = 2020.1, color = variable_col),
            hjust = "left",
            fontface = "bold",
            size = 3) +
  ...
  labs(title = "Top 100 dog friendly cities in the US",
       subtitle = "US Midwest cities are highlighted. <b style='color:#942DC7;'>Madison, WI</b> is the top friendly city in the Midwest",
       caption = "Source =  | Vizualization: @maxwelco") 
...
parks2 %>% 
  ...
  scale_x_continuous(breaks = seq(2015, 2020, 1), limits= c(2015, 2020), 
                     expand = c(.05, .05)) +
  geom_text(data = parks2 %>% filter(year == "2015"),
            mapping = aes(label = city, x = 2014.99, color = variable_col),
            hjust = "right",
            fontface = "bold",
            size = 2) +
  geom_text(data = parks2 %>% filter(year == "2020"),
