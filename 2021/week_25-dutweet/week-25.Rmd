...
```
filter(like_count == 636)

```{r}
tweets %>% 
  filter(!is.na(quote_count)) %>% 
  summarise(max(quote_count))
```

```{r}
tweets %>% 
  mutate(datetime = make_datetime(datetime))
```
...
tweets1 %>% 
  ...
  theme(plot.title = element_markdown(),
        ...
        plot.caption = element_markdown(color = "grey50")) -> fig1
fig1
```
