...
bey_tokens <- beyonce_lyrics %>%
  ...
  select(artist = artist_name,
         words = words)
#artist_tokens <- rbind(bey_tokens, taylor_tokens)
```
...
```{r}
wordcloud(words = cloud$word, freq =cloud$freq, min.freq=1, scale=c(4,.1),
          random.order=FALSE, rot.per=0.35, colors=pal) + facet_grid(~artist)
```
